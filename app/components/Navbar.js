...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Set hasScrolled if user has scrolled past 100px
  ...
  return (
    <div className={`fixed z-[999] w-full px-4 md:px-8 lg:px-15 py-2 md:py-3 font-["Neue Montreal"] flex justify-between items-center transition-all duration-300 ${
      isVisible ? 'translate-y-0' : '-translate-y-full'
    } ${
      hasScrolled ? 'backdrop-blur-sm bg-zinc-900/5' : 'bg-transparent'
    }`}>
        <div className='logo'>
        ...
        <div className='links flex gap-6 md:gap-10'>
            {["Service","Our Work","About Us","Insights","Contact"].map((item,index)=>(
                <a key={index} className={`text-sm md:text-md font-light hover:text-zinc-300 transition-colors ${index===4 && "md:ml-34"}`}>{item}</a>
            ))}
